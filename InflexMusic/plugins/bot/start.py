...
async def start_pm(client, message: Message, _):
    ...
    if len(message.text.split()) > 1:
        ...
        if name[0:3] == "sud":
            ...
            if await is_on_off(2):
                return await app.send_message(
                    ...
                    text=f"{message.from_user.mention} mahnı məlumatına baxdı. <b>Sudo istifadəçi</b>.\n\n<b>🆔:</b> <code>{message.from_user.id}</code>\n<b>👤:</b> @{message.from_user.username}",
                )
            return
        if name[0:3] == "inf":
            m = await message.reply_text("📥 Məlumatları göndərirəm...")
            ...
            if await is_on_off(2):
                return await app.send_message(
                    chat_id=config.LOG_GROUP_ID,
                    text=f"{message.from_user.mention} <b>mahnı məlumatına baxdı.</b>\n\n<b>🆔:</b> <code>{message.from_user.id}</code>\n<b>👤:</b> @{message.from_user.username}",
                )
    else:
        out = private_panel(_)
        ...
        if await is_on_off(2):
            return await app.send_message(
                chat_id=config.LOG_GROUP_ID,
                text=f"{message.from_user.mention} <b>botu start etdi.</b>\n\n<b>🆔:</b> <code>{message.from_user.id}</code>\n<b>👤:</b> @{message.from_user.username}",
            )
